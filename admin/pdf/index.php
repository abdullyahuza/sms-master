...
if(isset($_GET['print_result'])){
	...
	class PDF extends PDF_Rotate {
		...
		function Header(){
	        ...
	        //logo
	        $this->Image('logo.jpeg',5,3,30,30);
			$this->SetY(10);
		...
		function ClassCell($class){
			$this->Ln(0);
	        $this->SetFont('Arial','',12);
	        $w = $this->GetStringWidth($class)+18;
	        $this->SetX((210-$w)/2);
	        $this->SetTextColor(41,22,111);
	...
	$pdf->TotalScore(
		totalCA1($resultArr)." / ".count($resultArr)*20,
		totalCA2($resultArr)." / ".count($resultArr)*20,
		totalExams($resultArr)." / ".count($resultArr)*60,
		totalMarks($resultArr)." / ".count($resultArr)*100);
